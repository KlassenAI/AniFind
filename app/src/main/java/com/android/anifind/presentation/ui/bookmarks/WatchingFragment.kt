...
import com.android.anifind.databinding.FragmentWatchingBinding
...
class WatchingFragment : BaseBookmarksFragment(R.layout.fragment_watching) {

    private val binding: FragmentWatchingBinding by viewBinding()
    private val bookmarksViewModel: BookmarksViewModel by activityViewModels()
    private lateinit var adapter: AnimeAdapter

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) = with(binding) {
        super.onViewCreated(view, savedInstanceState)
        adapter = AnimeAdapter(bookmarksViewModel, this@WatchingFragment)
        recycler.init(adapter)
        bookmarksViewModel.watchingAnimes.observe(viewLifecycleOwner) {
